...
3. `.env.local`을 편집하고 API 키를 추가합니다:
   ...
   **UI 언어 설정**: `NEXT_PUBLIC_LOCALE` 변수는 사용자 인터페이스의 언어를 제어합니다. 다음 값으로 설정할 수 있습니다:
   - `en` - 영어（기본값）
   - `ja` - 일본어
   - `ko` - 한국어
   - `zh` - 중국어
   
   로케일을 변경한 후 변경 사항을 적용하려면 개발 서버를 다시 시작하세요.

   **Webhook 서명 검증**（선택사항）: `WEBHOOK_SECRET` 변수는 HMAC-SHA256을 사용한 Webhook 서명 검증을 활성화합니다. 설정된 경우 모든 수신 Webhook 요청이 이 시크릿에 대해 검증됩니다. Webhook 시크릿은 k-ID Compliance Studio의 제품 설정에서 가져올 수 있습니다. 제공되지 않은 경우 Webhook 서명 검증이 건너뜁니다（서명 검증 없이 테스트하는 데 유용합니다）.
...
이 명령은 Next.js 개발 서버와 ngrok 터널을 모두 시작합니다:

- 서버가 `http://localhost:3100`에서 실행됩니다
- Ngrok 터널이 외부 HTTPS URL을 생성합니다
- Webhook URL: `https://[random].ngrok-free.app/api/webhook`

**참고**: ngrok 계정 및 인증 토큰이 없는 한, 개발 서버를 재시작할 때마다 ngrok URL이 변경됩니다.

**📱 모바일 액세스를 위한 QR 코드**: QR 코드가 자동으로 생성되어 "Public Tunnel Access" 섹션에 표시됩니다. 휴대폰으로 이 QR 코드를 스캔하면 모바일 기기에서 k-ID Dev Explorer에 액세스할 수 있어 모바일 기기에서 직접 CDK 플로우를 테스트할 수 있습니다.

### ⚠️ 중요: WebAuthn 요구사항

로컬에서 개발할 때 **HTTPS로 실행하지 않으면 연령 키 생성 및 검증이 작동하지 않습니다**. 이는 WebAuthn의 요구사항입니다. ngrok 터널은 HTTPS 액세스를 제공하며, 연령 키 생성 또는 검증을 포함하는 플로우를 테스트하는 데 필요합니다.
...
- **서명 검증**: 각 Webhook 이벤트에 대한 Webhook 서명 검증 상태가 표시됩니다:
  - ✓ **서명됨**: Webhook 서명이 유효합니다（녹색 배지）
  - ✗ **잘못된 서명**: Webhook 서명이 일치하지 않습니다（빨간색 배지）
  - ⚠ **서명 없음**: Webhook 요청에 서명 헤더가 없습니다（노란색 배지）
  - ○ **서명 검증이 구성되지 않음**: `WEBHOOK_SECRET`이 설정되지 않았습니다（회색 배지）
- **복사 기능**: 복사 버튼을 클릭하여 Webhook 데이터를 클립보드에 복사

### Webhook 서명 검증

애플리케이션은 HMAC-SHA256을 사용한 Webhook 서명 검증을 지원합니다. 서명 검증을 활성화하려면:

1. [k-ID Compliance Studio](https://portal.k-id.com)의 제품 설정에서 Webhook 시크릿을 가져옵니다
2. `.env.local` 파일에 `WEBHOOK_SECRET`을 추가합니다:
   ```bash
   WEBHOOK_SECRET=your_webhook_secret_here
   ```
3. 개발 서버를 다시 시작합니다

`WEBHOOK_SECRET`이 구성된 경우 애플리케이션은:
- HMAC-SHA256을 사용하여 Webhook 서명을 검증합니다
- 서명 헤더를 확인합니다: `x-k-id-signature`, `x-signature` 또는 `k-id-signature`
- 각 Webhook 이벤트의 이벤트 로그에 서명 검증 상태를 표시합니다

`WEBHOOK_SECRET`이 제공되지 않은 경우 Webhook 서명 검증이 건너뜁니다（서명 검증 없이 테스트하는 데 유용합니다）.
...
- `NEXT_PUBLIC_LOCALE` - UI 언어 코드(선택사항, 기본값: 'en')
  - 지원 값: `en`（영어）、`ja`（일본어）、`ko`（한국어）、`zh`（중국어）
- `PORT` - 서버 포트(기본값: 3100, 선택사항)
- `NEXT_PUBLIC_APP_URL` - 로컬 URL 재정의(선택사항)
- `WEBHOOK_SECRET` - 서명 검증용 Webhook 시크릿(선택사항)
  - 설정된 경우 HMAC-SHA256 서명 검증을 사용하여 Webhook 요청이 검증됩니다
  - [k-ID Compliance Studio](https://portal.k-id.com)의 제품 설정에서 Webhook 시크릿을 가져옵니다
  - 제공되지 않은 경우 서명 검증이 건너뜁니다（테스트에 유용합니다）

### 스크립트

- `npm run dev` - ngrok 터널과 함께 개발 서버 시작
